...
namespace oct::verso::v1::devel
{
	...
	void Body::move(SDL_Rect& square, Circle& circle)
	{
		//Move the dot left or right
		mPosX += mVelX;
		shiftColliders();
	...
	void Body::render(LTexture& gDotTexture, SDL_Renderer* r)
	{
		//Show the dot
		gDotTexture.render(mPosX - mCollider.r, mPosY - mCollider.r, r);
	}

	Circle& Body::getCollider()
	{
		return mCollider;
	}
	...
	void Body::set(int x, int y, float m, float d)
	{
		mPosX = x;
		mPosY = y;
		mass = m;
	...
	float Body::gravitational(const Body& b)
	{
		const float G = 6.672e-11;
		float f = G * mass * b.mass;
		f = f / (b.distance * b.distance);
		return f;
	}
	std::vector<int> Body::gravitational_vector(const Body& b)
	{
		std::vector<int> v;
	...
	SolarI::SolarI()
	{
		sun.set(SCREEN_WIDTH / 2, SCREEN_WIDTH / 2, 1.989e30f,0.0f);
		mercury.set((SCREEN_WIDTH / 2) + 50, SCREEN_HEIGHT / 2, 3.3e23, 46001200.0f);

	}
	bool SolarI::initialize()
	{
		//Initialize SDL
		if (SDL_Init(SDL_INIT_VIDEO) < 0)
		{
	...
	void SolarI::loop()
	{
		...
		//Render dots
		sun.render(gDotTexture, renderer);
		mercury.render(gDotTexture, renderer);

		//Update screen
		SDL_RenderPresent(renderer);
	}
	bool SolarI::loadMedia()
	{
		//Load dot texture
		if (!gDotTexture.loadFromFile("../../../../devel/assets/dot.bmp", renderer))
		{
